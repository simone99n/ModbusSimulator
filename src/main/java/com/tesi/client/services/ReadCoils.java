...
public class ReadCoils {
    ...
    public boolean[] readCoils(RequestRead request) throws IOException {
        ...
        if(((receiveData[7] & 0xff) == 0x81) && (receiveData[8] == 0x03)){
            System.out.println("Quantity invalid");
            throw new IllegalStateException("Quantity invalid");
        }

        if(((receiveData[7] & 0xff) == 0x81) && (receiveData[8] == 0x04)){
        ...
        for (int i = 0; i < quantity; i++) {
            ...
            int mask = (int)Math.pow(2, (i%8));  //algoritmo complicato
