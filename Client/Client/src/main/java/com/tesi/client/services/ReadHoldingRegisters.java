...
public class ReadHoldingRegisters {
    ...
    public int[] readHoldingRegisters(RequestRead request) throws IOException {
        ...
        //unitIdentifier = 1 : already declared;
        ...
        if(((receiveData[7] & 0xff) == 0x83) && (receiveData[8] == 0x03)){
            System.out.println("Quantity invalid");
            throw new IllegalStateException("Quantity invalid");
        }

        if(((receiveData[7] & 0xff) == 0x83) && (receiveData[8] == 0x04)){
